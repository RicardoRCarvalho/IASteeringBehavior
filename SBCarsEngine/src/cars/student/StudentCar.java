...
public class StudentCar extends Car {
    ...
    public Vector2 seek(final World world){
        ...
        if (Sf != null) {
            ...
            acel.x *= -1;
            acel.y *=-1;
            return vec2(acel.x, acel.y);
        }
        else {
            return vec2();
        }
    }
    public Vector2 flee(final World world){
        double Sx = getPosition().x;
        double Sy = getPosition().y;
    ...
    public Vector2 calculateSteering(final World world) {
        Vector2 seekVector = seek(world);
        Vector2 fleeVector = flee(world);
